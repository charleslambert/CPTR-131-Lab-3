require 'socket'

class Server

	def initialize()
		@server = TCPServer.new 2000
		@client_hash = Hash.new
	end

	def server_run()
		loop do
  			Thread.start(@server.accept) do |client|

  				connect(client, @client_hash)

  				c_message = ""

  				while(c_message != "DISCONNECTED")
  					c_message = client.gets
  					c_message = command(c_message.strip, $1, client, @client_hash)
  					client.puts c_message
  				end

  				client.close
  			end
		end

	end


	def command(recieved_m, username, client, client_hash)

		case recieved_m

		when /BROADCAST\s+(.++)/
			return_m = broadcast($1, client_hash)
		when /SEND\s+(\S+)\s+(.+)/
			return_m = send(client_hash, $1, $2)
		when /USERLIST/
			return_m = userlist(client_hash, client)
		when /DISCONNECT/
			return_m = disconnect(client_hash, client, username)
		else
			return_m = "Not a valid command"
		end

		return return_m
	end

	def connect(client, client_hash)
		connected = false

		while(connected == false)

			c_message = client.gets
			
			if (/CONNECT\s+(\S+)/ =~ c_message)
				puts $1
				client_hash[$1] = client
				client.puts "CONNECTED"
				connected = true
			else
				client.puts "FAILED"
				connected = false
			end
		end
	end

	def broadcast(message, client_hash)
		client_hash.each do |key, value|
			value.puts message
		end
		return "SENT"
	end

	def send(client_hash, user, message)
		if (client_hash.has_key?(user))
			client_hash[user].puts "SENTFROM #{username} #{message}"
			return "SENT"
		else 
			return "#{user} is not a user."
		end
	end

	def userlist(client_hash, client)
		client.puts "USERS #{client_hash.keys.inspect}"
		return "SENT"
	end

	def disconnect(client_hash, client, username)
		client_hash.delete(username)
		return "DISCONNECTED"
	end

end

server = Server.new
server.server_run